...
const RadiationCanvas = forwardRef(function RadiationCanvas(
  ...
  useImperativeHandle(
    ref,
    () => ({
      setOpening: (opening) => engineRef.current?.setOpening(opening),
  ...
  useEffect(() => {
    ...
    const applyVisibility = () => engine.setVisible(inView && !document.hidden);
    ...
    const handleVisibility = applyVisibility;

    const handleReducedChange = (event) => engine.setReducedMotion(event.matches);
    const handleMobileChange = (event) => engine.setDensity(event.matches ? 0.62 : 1);
